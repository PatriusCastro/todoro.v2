      ...
      <div
        ...
        <div className="w-full max-w-md panel bg-panel shadow-lg flex flex-col gap-4 p-5 max-h-[90dvh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between">
            <h2 className="font-semibold text-lg text-tx">Rewards</h2>
          ...
          <div className="flex flex-col gap-2 rounded-2xl border border-border bg-surface2 px-4 py-3">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-tx">Level {level}</span>
              <span className="text-xs text-sub">{levelInto} / {levelSpan} to next</span>
            </div>
            <div className="h-2 rounded-full bg-ring overflow-hidden">
              <div className="h-full rounded-full bg-tx transition-all duration-500" style={{ width: `${levelPct}%` }} />
            </div>
          </div>
          ...
          {canRestore && (
            <div className="flex flex-col gap-2 rounded-2xl border border-priority-low/40 bg-priority-low/5 px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-tx">Your streak broke</span>
              </div>
              <p className="text-xs text-sub">Use a Streak Freeze to bridge the day you missed and bring your streak back.</p>
              <button
                onClick={onRestore}
                disabled={freezes < 1}
                className="mt-1 py-2.5 rounded-xl bg-priority-low text-white text-sm font-semibold hover:bg-[#42c956] disabled:opacity-40 transition-all">
          ...
          {/* Streak Freeze item */}
          <div className="flex flex-col gap-3 rounded-control border border-border bg-surface px-4 py-4">
            <div className="flex items-start gap-3">
              <div className="w-11 h-11 rounded-xl bg-accent/15 flex items-center justify-center shrink-0">
                <HiFire size={20} className="text-accent" />
              </div>
              <div className="flex-1">
                <p className="text-sm font-semibold text-tx">Streak Freeze</p>
            ...
            <button
              onClick={onBuyFreeze}
              disabled={!canAfford}
              className="py-2.5 rounded-xl bg-accent text-white text-sm font-semibold hover:bg-accent-hover disabled:opacity-40 transition-all">
